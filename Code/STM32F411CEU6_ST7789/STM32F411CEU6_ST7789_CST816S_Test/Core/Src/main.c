...
/**
 * @brief  Transmit a formatted string over UART1.
 */
...
/**
 * @brief  HAL EXTI callback – must be at file scope, NOT inside main().
 */
...
int main(void)
{
  ...
  /* ── Fix: use the correct CubeMX-generated GPIO defines ─────────────────── */
  ...
  /* Optional: print chip info once at startup */
  {
      uint8_t chip_id = 0, proj_id = 0, fw_ver = 0;
      if (CST816S_GetInfo(&touch, &chip_id, &proj_id, &fw_ver))
      {
          char info[64];
          snprintf(info, sizeof(info),
                   "ChipID=0x%02X  ProjID=0x%02X  FW=0x%02X\r\n",
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOA, RST_Pin|TFT_CS_Pin|DC_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, LCD_BL_Pin|C_RST_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pins : RST_Pin TFT_CS_Pin DC_Pin */
  GPIO_InitStruct.Pin = RST_Pin|TFT_CS_Pin|DC_Pin;
